...
describe PgQuery, '.parse' do
  def build_raw_stmt(node_kind, node)
    PgQuery::Node.new(
      list: PgQuery::List.new(
        items: [
          PgQuery::Node.new(
            raw_stmt: PgQuery::RawStmt.new(
  ...
  it "parses a simple query" do
    ...
    expect(JSON.parse(PgQuery::Node.encode_json(query.tree))).to eq({
      "list" => {
        "items" => [
          {
            "rawStmt" => {
  ...
  it "handles errors" do
    expect { described_class.parse("SELECT 'ERR") }.to(raise_error do |error|
      ...
      expect(error.message).to eq "unterminated quoted string at or near \"'ERR\" (scan.l:1204)"
  ...
  it "parses empty queries" do
    ...
    expect(query.tree).to eq PgQuery::Node.new
  ...
  it "parses floats with leading dot" do
    ...
    expr = q.tree.list.items[0].raw_stmt.stmt.select_stmt.target_list[0].res_target.val
  ...
  it "parses ALTER TABLE" do
    ...
    expect(query.tree).to eq(build_raw_stmt(
      :alter_table_stmt, PgQuery::AlterTableStmt.new(
        ...
        cmds: [
          PgQuery::Node.new(
            alter_table_cmd: PgQuery::AlterTableCmd.new(subtype: :AT_AddConstraint, def: PgQuery::Node.new(
              constraint: PgQuery::Constraint.new(contype: :CONSTR_PRIMARY, location: 21, keys: [
                PgQuery::Node.new(string: PgQuery::String.new(str: 'gid'))
              ])
            ))
          )
        ],
  ...
  it 'parses CREATE SCHEMA' do
    ...
    expect(query.tree).to eq(build_raw_stmt(
      :create_schema_stmt, PgQuery::CreateSchemaStmt.new(
        schemaname: 'test',
        authrole: PgQuery::RoleSpec.new(roletype: 0, rolename: 'joe', location: 47),
        if_not_exists: true
      )
    ))
  end
      ...
      :create_function_stmt, PgQuery::CreateFunctionStmt.new(
        ...
        parameters: [
          PgQuery::Node.new(function_parameter: PgQuery::FunctionParameter.new(
            ...
            mode: 'i'.ord # FIXME: There is an issue here with how the FunctionParameterMode enum is translated (enum values should be the character values)
          ))
        ],
        ...
        parameters: [
          ...
          PgQuery::Node.new(
            function_parameter: PgQuery::FunctionParameter.new(
              ...
              mode: 't'.ord # FIXME: There is an issue here with how the FunctionParameterMode enum is translated (enum values should be the character values)
            )
          )
        ],
        return_type: PgQuery::TypeName.new(
